...
@OpenAPIDefinition(info = @Info(description = "С помощью данного API можно работать с опросами. Есть две роли, админа и пользователя. \n" +
        ...
        "\n" +
        "\n" +
        "ВНИМАНИЕ: API расположен по мэппингу \"/api\", так что этот префик должен быть перед каждым запросом. " +
        "Это обозначено в сервере на странице ниже, но это легко можно незаметить.",
